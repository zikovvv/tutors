...
class ImmutableBuffer:
    ...
    def wait(self, timeout = -1, raise_exception_on_timeout : bool = False):
        if self.lock.acquire(timeout = timeout) :
            res =  self.item
            self.lock.release()
            return res
        elif raise_exception_on_timeout :
            raise TimeoutError()
...
def hashfile(path) :
    with open(path, 'rb') as f :
        return hashbytes(f.read())

def strip_exif(path, dest = None, resize_koef = 0.02) -> None :
    ...
    if resize_koef :
        image_without_exif = image_without_exif.resize((int(size[0] * (1 - resize_koef + random.random() * resize_koef * 2)),
                                                        int(size[1] * (1 - resize_koef + random.random() * resize_koef * 2))),
                                                       Image.ANTIALIAS)
    image_without_exif = image_without_exif.convert('RGB')
    image_without_exif.save(path if dest is None else dest)
...
def run_many_threaded_functions_and_await_all_output(functions_args_kwargs, timeout = None, exception_on_timeout = None):
    buffers : list[ImmutableBuffer] = [ImmutableBuffer() for i in range(len(functions_args_kwargs))]
    threads : list[Thread] = []
    for buffer, (func, args, kwargs) in zip(buffers, functions_args_kwargs) :
        t = Thread(
                target = lambda *args, **kwargs: buffer.set(func(*args, **kwargs)),
                args = args,
                kwargs = kwargs,
                daemon = True
            )
    ...
    for buff, th in zip(buffers, threads) :
        th.join(timeout = timeout)
        if th.is_alive() :
            if exception_on_timeout is not None :
                raise exception_on_timeout()
            else :
                buff.set(None)
...
def remove_repeats(text : str, chars = '') -> str :
    return reduce(lambda a, b : a if b == a[-1] and (b in chars if chars else True) else a + b, text) if len(text) >= 1 else text

def normalize_text(text : str, allowed_symbols : str = '', norepeat = False, bad_symbols_replacement = ' ', nodigit = False) -> str :
    allowed_symbols += ('' if nodigit else "123457890") + "qwertyuiopasdfghjklzxcvbnm "
    return remove_repeats(
        ''.join(
            a if a and a in allowed_symbols else bad_symbols_replacement for a in map(lambda x : unidecode(x).lower(), text)
        ).strip(),
        '' if norepeat else bad_symbols_replacement
    ) if text else text

def get_arg_from_dict_array(arr: typing.Iterable[Dict or List or Tuple], key: str):
    return list(map(itemgetter(key), arr))

def run_function_threaded(buff: ImmutableBuffer, func, *args, **kwargs):
    Thread(target = lambda : buff.set(func(*args, **kwargs)), daemon = True).start()

def cmd(cmd:  str) -> str :
    with TempFile(postfix = '.txt', binary = False) as f :
        os.system(cmd + f" > {f.name}")
        return f.read()

def clip(nb, min_nb = None, max_nb = None, cycle = False) :
    if cycle and min_nb is not None and max_nb is not None :
        if nb < min_nb : return max_nb
        elif nb > max_nb : return min_nb
    elif min_nb is not None :
        return max(min_nb, nb)
    elif max_nb is not None :
        return min(max_nb, nb)

def kill_window_by_id(disp : int, wid : int) :
    os.system(f'DISPLAY=:{disp} xkill -id {wid}')
